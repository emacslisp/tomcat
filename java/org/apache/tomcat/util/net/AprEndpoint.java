...
public class AprEndpoint extends AbstractEndpoint<Long, Long> implements SNICallBack {
	...
	protected void createSSLContext(SSLHostConfig sslHostConfig) throws Exception
	{
		...
		for (SSLHostConfigCertificate certificate : certificates) {
			...
			if (firstCertificate) {
				...
				List<String> enabledProtocols = SSLUtilBase.getEnabled("protocols", log, true,
						sslHostConfig.getProtocols(), OpenSSLEngine.IMPLEMENTED_PROTOCOLS_SET);
				sslHostConfig.setEnabledProtocols(enabledProtocols.toArray(new String[enabledProtocols.size()]));
				// Configure the enabled ciphers
				List<String> enabledCiphers = SSLUtilBase.getEnabled("ciphers", log, false,
						sslHostConfig.getJsseCipherNames(), OpenSSLEngine.AVAILABLE_CIPHER_SUITES);
	 ...
	 *            The size
	 * @param pool
	 *            The pool from which the poller will be allocated
	 * @param timeout
	 *            The timeout
	...
	public class Poller implements Runnable {
		...
		protected synchronized void init()
		{
			...
			/*
			 * x2 - One descriptor for the socket, one for the event(s). x2 -
			 * Some APR implementations return multiple events for the same
			 * socket as different entries. Each socket is registered for a
			 * maximum of two events (read and write) at any one time.
			 *
		 ...
		 * @param timeout
		 *            to use for this connection in milliseconds
		 * @param flags
		 *            Events to poll for (Poll.APR_POLLIN and/or
		 *            Poll.APR_POLLOUT)
		 */
					...
					for (int i = 0; i < pollers.length; i++) {
						...
						if (rv > 0) {
							...
							for (int n = 0; n < rv; n++) {
								...
								if (wrapper == null) {
									// Socket was closed in another thread while
									// still in
		...
		private int mergeDescriptors(long[] desc, int startCount)
		{
			 ...
			 * same, or a similar, issue is happening on Windows. Notes: Only
			 * the first startCount * 2 elements of the array are populated. The
			 * array is event, socket, event, socket etc.
			 */
