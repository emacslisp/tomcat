...
public class Nio2Endpoint extends AbstractJsseEndpoint<Nio2Channel, AsynchronousSocketChannel> {
	 ...
	 *         processing may continue, <code>false</code> if the socket needs
	...
	private void closeSocket(SocketWrapperBase<Nio2Channel> socket)
	{
		...
		try {
			...
			if (nio2Socket.getSendfileData() != null && nio2Socket.getSendfileData().fchannel != null
	...
	public static class Nio2SocketWrapper extends SocketWrapperBase<Nio2Channel> {
				...
				public void completed(Integer nBytes, ByteBuffer attachment)
				{
					...
					synchronized (writeCompletionHandler) {
						if (nBytes.intValue() < 0) {
							...
							getSocket().write(array, 0, array.length, getNio2WriteTimeout(), TimeUnit.MILLISECONDS,
									array, gatheringWriteCompletionHandler);
							nestedWriteCompletionCount.get().decrementAndGet();
						} else if (attachment.hasRemaining()) {
							// Regular write
							nestedWriteCompletionCount.get().incrementAndGet();
							getSocket().write(attachment, getNio2WriteTimeout(), TimeUnit.MILLISECONDS, attachment,
									writeCompletionHandler);
							nestedWriteCompletionCount.get().decrementAndGet();
						} else {
		...
		private static class OperationState<A> {
			...
			private OperationState(ByteBuffer[] buffers, int offset, int length, BlockingMode block, long timeout,
		...
		private class ScatterReadCompletionHandler<A> implements CompletionHandler<Long, OperationState<A>> {
			...
			public void completed(Long nBytes, OperationState<A> state)
			{
				if (nBytes.intValue() < 0) {
					...
					CompletionState currentState = Nio2Endpoint.isInline() ? CompletionState.INLINE
					...
					if (complete) {
						...
						getSocket().read(state.buffers, state.offset, state.length, state.timeout, state.unit, state,
								this);
					}
				}
			}
			...
			public void completed(Long nBytes, OperationState<A> state)
			{
				if (nBytes.longValue() < 0) {
					...
					if (complete) {
						...
						getSocket().write(state.buffers, state.offset, state.length, state.timeout, state.unit, state,
								this);
					}
				}
			}
				...
				TimeUnit unit, A attachment, CompletionCheck check, CompletionHandler<Long, ? super A> handler)
		{
			...
			OperationState<A> state = new OperationState<>(dsts, offset, length, block, timeout, unit, attachment,
					check, handler);
				...
				TimeUnit unit, A attachment, CompletionCheck check, CompletionHandler<Long, ? super A> handler)
		{
			...
			OperationState<A> state = new OperationState<>(srcs, offset, length, block, timeout, unit, attachment,
					check, handler);
		...
		/*
		 * Callers of this method must: - have acquired the readPending
		 * semaphore - have acquired a lock on readCompletionHandler
		 *
		...
		private int fillReadBuffer(boolean block, ByteBuffer to) throws IOException
		{
			...
			if (block) {
				...
				Nio2Endpoint.startInline();
				getSocket().read(to, getNio2ReadTimeout(), TimeUnit.MILLISECONDS, this, readCompletionHandler);
				Nio2Endpoint.endInline();
		...
		/**
		 * @param block
		 *            Ignored since this method is only called in the blocking
		 *            case
		 */
		@Override
			...
			synchronized (writeCompletionHandler) {
				if (hasPermit || writePending.tryAcquire()) {
					...
					if (bufferedWrites.size() > 0) {
						...
						ByteBuffer[] array = arrayList.toArray(new ByteBuffer[arrayList.size()]);
						Nio2Endpoint.startInline();
						getSocket().write(array, 0, array.length, getNio2WriteTimeout(), TimeUnit.MILLISECONDS, array,
		...
		{
			synchronized (writeCompletionHandler) {
				return !socketBufferHandler.isWriteBufferEmpty() || bufferedWrites.size() > 0 || getError() != null;
			}
		}
		...
		public void awaitBytes()
		{
			...
			if (readPending.tryAcquire()) {
				...
				Nio2Endpoint.startInline();
				getSocket().read(getSocket().getBufHandler().getReadBuffer(), getNio2ReadTimeout(),
		...
		public SendfileState processSendfile(SendfileDataBase sendfileData)
		{
			...
			if (nRead >= 0) {
				...
				getSocket().getBufHandler().configureWriteBufferForRead();
				Nio2Endpoint.startInline();
				getSocket().write(buffer, getNio2WriteTimeout(), TimeUnit.MILLISECONDS, data, sendfileHandler);
				Nio2Endpoint.endInline();
				if (data.doneInline) {
					if (data.error) {
						return SendfileState.ERROR;
