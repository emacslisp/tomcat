...
public abstract class AbstractEndpoint<S, U> {
	 ...
	 *
	 * @param sslHostConfig
	 *            The SSLHostConfig for which the SSLContext should be created
	 * @throws Exception
			...
			for (int i = 0; i < unlocksRequired; i++) {
				try (java.net.Socket s = new java.net.Socket()) {
					...
					if (getDeferAccept()) {
						 ...
						 * need to send data to wake up the accept. Send OPTIONS
						 * * to bypass even BSD accept filters. The Acceptor
						 * will discard it.
						 */
	 ...
	 * @param event
	 *            The socket event to be processed
	 * @param dispatch
