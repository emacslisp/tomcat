...
public abstract class SocketWrapperBase<E> {
	 ...
	 *
	 * @param runnable
	 *            The actions to process on a container thread
	 *
	 ...
	 *
	 * @param readTimeout
	 *            The timeout in milliseconds. A value of -1 indicates an
	 *            infinite timeout.
	 */
	 ...
	 *
	 * @param writeTimeout
	 *            The timeout in milliseconds. A value of zero or less indicates
	 *            an infinite timeout.
	 */
	 ...
	 * be called again until the pending write has completed and the callback
	 * has been fired. TODO: Modify {@link #registerWriteInterest()} so the
	 * above restriction is enforced there rather than relying on the caller.
	 *
	 ...
	 *
	 * @param block
	 *            <code>true</code> if a blocking write should be used,
	 *            otherwise a non-blocking write will be used
	 * @param buf
	 *            The byte array containing the data to be written
	 * @param off
	 *            The offset within the byte array of the data to be written
	 * @param len
	 ...
	 *
	 * @param block
	 *            Should the write be blocking or not?
	 *
	 * @throws IOException
	 ...
	 *
	 * @param sendfileData
	 *            Data representing the file to send
	 *
	 ...
	 *
	 * @param sslSupport
	 *            The SSL/TLS support instance currently being used by the
	...
	public enum BlockingMode {
		/**
		 * The operation will now block. If there are pending operations, the
		 * operation will return CompletionState.NOT_DONE.
		 */
		NON_BLOCK,
		/**
	...
	public interface CompletionCheck {
		 ...
		 *
		 * @param state
		 *            of the operation (done or done in-line since the IO call
		 *            is done)
		 * @param buffers
		 *            ByteBuffer[] that has been passed to the original IO call
		 * @param offset
		 *            that has been passed to the original IO call
		 * @param length
		 *            that has been passed to the original IO call
		 *
	...
	/**
	 * This utility CompletionCheck will cause the completion handler to be
	 * called once some data has been read. If the operation completes inline,
	 ...
	 *         read operation is being processed asynchronously
	 */
	 ...
	 * until the operation completes, or the specified amount of time has
	 * passed.
	 * 
	 * @param timeout
	...
	/**
	 * Scatter read. The completion handler will be called once some data has
	 * been read or an error occurred. If a CompletionCheck object has been
	 ...
	 * default NIO2 behavior is used: the completion handler will be called as
	 * soon as some data has been read, even if the read has completed inline.
	 *
	 * @param block
	 *            is the blocking mode that will be used for this operation
	 * @param timeout
	 *            timeout duration for the read
	 * @param unit
	 *            units for the timeout duration
	 * @param attachment
	 *            an object to attach to the I/O operation that will be used
	 *            when calling the completion handler
	 * @param check
	 *            for the IO operation completion
	 * @param handler
	 *            to call when the IO is complete
	 * @param dsts
	 *            buffers
	 * @param <A>
	 *            The attachment type
	 ...
	 * @return the completion state (done, done inline, or still pending)
	 */
	public <A> CompletionState read(ByteBuffer[] dsts, int offset, int length, BlockingMode block, long timeout,
	...
	/**
	 * Gather write. The completion handler will be called once some data has
	 * been written or an error occurred. If a CompletionCheck object has been
	 * provided, the completion handler will only be called if the callHandler
	 * method returned true. If no CompletionCheck object has been provided, the
	 * default NIO2 behavior is used: the completion handler will be called,
	 * even if the write is incomplete and data remains in the buffers, or if
	 * the write completed inline.
	 *
	 * @param block
	 *            is the blocking mode that will be used for this operation
	 * @param timeout
	 *            timeout duration for the write
	 * @param unit
	 *            units for the timeout duration
	 * @param attachment
	 *            an object to attach to the I/O operation that will be used
	 ...
	 * @return the completion state (done, done inline, or still pending)
	 */
	public <A> CompletionState write(ByteBuffer[] srcs, int offset, int length, BlockingMode block, long timeout,
			TimeUnit unit, A attachment, CompletionCheck check, CompletionHandler<Long, ? super A> handler)
	{
		throw new UnsupportedOperationException();
	}
