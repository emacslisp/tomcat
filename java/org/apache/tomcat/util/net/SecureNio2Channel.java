		...
		while (!handshakeComplete) {
			...
			case FINISHED: {
				if (endpoint.hasNegotiableProtocols()) {
					if (sslEngine instanceof SSLUtil.ProtocolInfo) {
						...
						socket.setNegotiatedProtocol(JreCompat.getInstance().getApplicationProtocol(sslEngine));
					}
				}
			...
			case NEED_UNWRAP: {
				...
				if (handshake.getStatus() == Status.OK) {
					...
					throw new IOException(
							sm.getString("channel.nio.ssl.unexpectedStatusDuringUnwrap", handshakeStatus));
				}
				break;
			}
			case NEED_TASK: {
						...
						do {
							...
							if (unwrap.getStatus() == Status.OK || unwrap.getStatus() == Status.BUFFER_UNDERFLOW) {
								...
								if (read > 0) {
									...
									// read data. Return
									// so the destination buffer can be emptied
									// before another
									// read is attempted
									break;
								} else {
							...
							// continue to unwrap as long as the input buffer
							// has stuff
