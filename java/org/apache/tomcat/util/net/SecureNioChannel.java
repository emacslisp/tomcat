...
public class SecureNioChannel extends NioChannel {
	...
	public SecureNioChannel(SocketChannel channel, SocketBufferHandler bufHandler, NioSelectorPool pool,
	 ...
	 * @param s
	 *            The selector to use for blocking, if null then a busy write
	 *            will be initiated
	 * @param timeout
	 *            The timeout for this write operation in milliseconds, -1 means
	 *            no timeout
	 ...
	 * IOException) occurred, otherwise it returns a SelectionKey interestOps
	 * value
	 *
	...
	public int read(ByteBuffer dst) throws IOException
	{
		...
		if (dst != getBufHandler().getReadBuffer()
				&& (getAppReadBufHandler() == null || dst != getAppReadBufHandler().getByteBuffer())) {
		...
		} while (netInBuffer.position() != 0); // continue to unwrapping as long
												// as the input buffer has stuff
		return read;
	}
