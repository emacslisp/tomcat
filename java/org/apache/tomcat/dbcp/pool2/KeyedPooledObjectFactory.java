 ...
 * {@link KeyedObjectPool#borrowObject borrowed} from the pool.</li>
 * <li>{@link #validateObject} may be invoked on {@link #activateObject
 * activated} instances to make sure they can be
 * {@link KeyedObjectPool#borrowObject borrowed} from the pool.
 * <code>validateObject</code> may also be used to test an instance being
 ...
 * <li>{@link #destroyObject destroyObject} is invoked on every instance when it
 * is being "dropped" from the pool (whether due to the response from
 * <code>validateObject</code>, or for reasons specific to the pool
 * implementation.) There is no guarantee that the instance being destroyed will
 * be considered active, passive or in a generally consistent state.</li>
 * </ol>
 * {@link KeyedPooledObjectFactory} must be thread-safe. The only promise an
 * {@link KeyedObjectPool} makes is that the same instance of an object will not
 ...
 * underlying value type V, the factory methods act on instances of
 * {@link PooledObject PooledObject&lt;V&gt;}. These are the object wrappers
 * that pools use to track and maintain state informations about the objects
 * that they manage.
 *
 * @see KeyedObjectPool
 ...
 *
 * @param <K>
 *            The type of keys managed by this factory.
 * @param <V>
