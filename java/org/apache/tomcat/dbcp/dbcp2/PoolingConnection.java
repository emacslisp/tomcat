 ...
 * from a pool of unused statements. The {@link PreparedStatement#close} method
 * of the returned statement doesn't actually close the statement, but rather
 * returns it to the pool. (See {@link PoolablePreparedStatement},
 * {@link PoolableCallableStatement}.)
 *
 *
...
public class PoolingConnection extends DelegatingConnection<Connection>
	 ...
	 * The <code>stmtType</code> field in the key determines whether a
	...
	public PooledObject<DelegatingPreparedStatement> makeObject(final PStmtKey key) throws Exception
	{
		...
		if (null == key.getResultSetType() && null == key.getResultSetConcurrency()
			...
			return new DefaultPooledObject<>(new PoolableCallableStatement(getDelegate().prepareCall(key.getSql(),
					key.getResultSetType().intValue(), key.getResultSetConcurrency().intValue()), key, _pstmtPool,
					this));
		}
	}
	 ...
	 * PoolablePreparedStatements and PoolableCallableStatements. Closes the
	...
	/**
	 * {@link KeyedPooledObjectFactory} method for validating pooled statements.
	 ...
	 * {@link PreparedStatement}s or {@link CallableStatement}s. Invokes
