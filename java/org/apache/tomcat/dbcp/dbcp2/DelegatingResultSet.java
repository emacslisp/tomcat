 ...
 * corresponding method on the "delegate" provided in my constructor.
 * <p>
 * Extends AbandonedTrace to implement result set tracking and logging of code
 * which created the ResultSet. Tracking the ResultSet ensures that the
 * Statement which created it can close any open ResultSet's on Statement close.
 *
...
public final class DelegatingResultSet extends AbandonedTrace implements ResultSet {
	...
	/**
	 * Create a wrapper for the ResultSet which traces this ResultSet to the
	 * Statement which created it and the code which created it.
	 * <p>
	 * Private to ensure all construction is
	 * {@link #wrapResultSet(Statement, ResultSet)}
	 *
	 * @param stmt
	 *            Statement which created this ResultSet
	 * @param res
	 *            ResultSet to wrap
	 */
	...
	/**
	 * Create a wrapper for the ResultSet which traces this ResultSet to the
	 * Connection which created it (via, for example DatabaseMetadata, and the
	...
	/**
	 * If my underlying {@link ResultSet} is not a {@code DelegatingResultSet},
	 ...
	 * {@code DelegatingResultSet}, or {@code null} when no
	 * non-{@code DelegatingResultSet} delegate can be found by transversing
	 * this chain.
	 * <p>
	 * This method is useful when you may have nested
	 * {@code DelegatingResultSet}s, and you want to make sure to obtain a
	 * "genuine" {@link ResultSet}.
	 * 
	...
	/**
	 * Wrapper for close of ResultSet which removes this result set from being
	 * traced then calls close on the original ResultSet.
	 */
	@Override
