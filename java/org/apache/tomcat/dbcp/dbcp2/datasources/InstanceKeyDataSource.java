 ...
 * <code>PerUserPoolDataSource</code>. Many of the configuration properties are
 * shared and defined here. This class is declared public in order to allow
 * particular usage with commons-beanutils; do not make direct use of it outside
 * of <em>commons-dbcp2</em>.
 * </p>
 *
 * <p>
 * A J2EE container will normally provide some method of initializing the
 * <code>DataSource</code> whose attributes are presented as bean
 * getters/setters and then deploying it via JNDI. It is then available to an
 * application as a source of pooled logical connections to the database. The
 * pool needs a source of physical connections. This source is in the form of a
 * <code>ConnectionPoolDataSource</code> that can be specified via the
 * {@link #setDataSourceName(String)} used to lookup the source via JNDI.
 * </p>
 *
 * <p>
 * Although normally used within a JNDI environment, A DataSource can be
 * instantiated and initialized as any bean. In this case the
 * <code>ConnectionPoolDataSource</code> will likely be instantiated in a
 * similar manner. This class allows the physical source of connections to be
 * attached directly to this pool using the
 * {@link #setConnectionPoolDataSource(ConnectionPoolDataSource)} method.
 * </p>
 *
 * <p>
 * The dbcp package contains an adapter,
 * {@link org.apache.tomcat.dbcp.dbcp2.cpdsadapter.DriverAdapterCPDS}, that can
 * be used to allow the use of <code>DataSource</code>'s based on this class
 * with JDBC driver implementations that do not supply a
 * <code>ConnectionPoolDataSource</code>, but still provide a
...
public abstract class InstanceKeyDataSource implements DataSource, Referenceable, Serializable, AutoCloseable {
	 ...
	 *         {@link GenericKeyedObjectPoolConfig#getBlockWhenExhausted()} for
	 *         each per user pool.
	 */
	...
	/**
	 * @return the default value for
	 *         {@link GenericKeyedObjectPoolConfig#getEvictionPolicyClassName()}
	...
	/**
	 * @return the default value for
	 *         {@link GenericKeyedObjectPoolConfig#getLifo()} for each per user
	 *         pool.
	 */
	...
	/**
	 * Sets the default value for {@link GenericKeyedObjectPoolConfig#getLifo()}
	 * for each per user pool.
	 * 
	 * @param lifo
	 *            The new value
	...
	/**
	 * @return the default value for
	 *         {@link GenericKeyedObjectPoolConfig#getMaxIdlePerKey()} for each
	 *         per user pool.
	 */
	...
	/**
	 * @return the default value for
	 *         {@link GenericKeyedObjectPoolConfig#getMaxTotalPerKey()} for each
	 *         per user pool.
	 */
	...
	/**
	 * @return the default value for
	 *         {@link GenericKeyedObjectPoolConfig#getMaxWaitMillis()} for each
	 *         per user pool.
	 */
	...
	/**
	 * @return the default value for
	 *         {@link GenericKeyedObjectPoolConfig#getMinEvictableIdleTimeMillis()}
	 *         for each per user pool.
	 */
	...
	/**
	 * @return the default value for
	 *         {@link GenericKeyedObjectPoolConfig#getMinIdlePerKey()} for each
	 *         per user pool.
	 */
	...
	/**
	 * @return the default value for
	 *         {@link GenericKeyedObjectPoolConfig#getNumTestsPerEvictionRun()}
	 *         for each per user pool.
	 */
	 ...
	 * Sets the default value for
	 * {@link org.apache.tomcat.dbcp.pool2.impl.GenericObjectPool
	 * GenericObjectPool#getTimeBetweenEvictionRunsMillis ()} for each per user
	 * pool.
	 * 
	...
	/**
	 * Get the value of connectionPoolDataSource. This method will return null,
	 * if the backing datasource is being accessed via jndi.
	 *
	...
	/**
	 * Set the backend ConnectionPoolDataSource. This property should not be set
	 * if using jndi to access the datasource.
	 *
	 * @param v
	...
	/**
	 * Get the name of the ConnectionPoolDataSource which backs this pool. This
	 * name is used to look up the datasource from a jndi service provider.
	 *
	...
	/**
	 * Set the name of the ConnectionPoolDataSource which backs this pool. This
	 * name is used to look up the datasource from a jndi service provider.
	 *
	 * @param v
	 ...
	 * connections handed out from this pool. The value can be changed on the
	 * Connection using Connection.setAutoCommit(boolean). The default is
	 * <code>null</code> which will use the default value for the drive.
	 *
	...
	/**
	 * Get the value of defaultReadOnly, which defines the state of connections
	 * handed out from this pool. The value can be changed on the Connection
	 * using Connection.setReadOnly(boolean). The default is <code>null</code>
	 * which will use the default value for the drive.
	 *
	...
	/**
	 * Set the value of defaultReadOnly, which defines the state of connections
	 * handed out from this pool. The value can be changed on the Connection
	 * using Connection.setReadOnly(boolean). The default is <code>null</code>
	 * which will use the default value for the drive.
	 *
	 ...
	 * connections handed out from this pool. The value can be changed on the
	 * Connection using Connection.setTransactionIsolation(int). If this method
	 * returns -1, the default is JDBC driver dependent.
	 *
	 ...
	 * connections handed out from this pool. The value can be changed on the
	 * Connection using Connection.setTransactionIsolation(int). The default is
	...
	/**
	 * Get the description. This property is defined by JDBC as for use with GUI
	 * (or other) tools that might deploy the datasource. It serves no internal
	 * purpose.
	 *
	...
	/**
	 * Set the description. This property is defined by JDBC as for use with GUI
	 * (or other) tools that might deploy the datasource. It serves no internal
	 * purpose.
	 *
	 * @param v
	...
	/**
	 * Get the value of jndiEnvironment which is used when instantiating a jndi
	 * InitialContext. This InitialContext is used to locate the backend
	 * ConnectionPoolDataSource.
	 * 
	 * @param key
	 *            The environment property name
	 ...
	 *
	 * @param properties
	 *            the JNDI environment property to set which will overwrite any
	 *            current settings
	 */
	...
	/**
	 * Whether a rollback will be issued after executing the SQL query that will
	 * be used to validate connections from this pool before returning them to
	 * the caller.
	 *
	 * @return true if a rollback will be issued after executing the validation
	 *         query
	 */
	 ...
	 * Whether a rollback will be issued after executing the SQL query that will
	 * be used to validate connections from this pool before returning them to
	 * the caller. Default behavior is NOT to issue a rollback. The setting will
	 * only have an effect if a validation query is set
	 *
	 ...
	 * {@link #getPooledConnectionAndInfo(String, String)} with the provided
	 * username and password. The password on the
	 * {@link PooledConnectionAndInfo} instance returned by
	 * <code>getPooledConnectionAndInfo</code> is compared to the
	 * <code>password</code> parameter. If the comparison fails, a database
	 * connection using the supplied username and password is attempted. If the
	 * connection attempt fails, an SQLException is thrown, indicating that the
	 * given password did not match the password used to create the pooled
	 * connection. If the connection attempt succeeds, this means that the
	 * database password has been changed. In this case, the
	 * <code>PooledConnectionAndInfo</code> instance retrieved with the old
	 * password is destroyed and the <code>getPooledConnectionAndInfo</code> is
	...
	public Connection getConnection(final String username, final String password) throws SQLException
	{
		...
		if (!(null == password ? null == info.getPassword() : password.equals(info.getPassword()))) { // Password
																										// on
			 ...
			 * retrying until we get a new, good one, destroying any idle
			 * connections with the old password as we pull them from the pool.
			 */
			...
			info = null;
			for (int i = 0; i < 10; i++) { // Bound the number of retries - only
											// needed if bad instances return
				try {
					info = getPooledConnectionAndInfo(username, password);
				} catch (final NoSuchElementException e) {
					closeDueToException(info);
