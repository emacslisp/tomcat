 ...
 * <p>
 * All of the methods from the {@link Connection} interface simply check to see
 * that the {@link Connection} is active, and call the corresponding method on
 * the "delegate" provided in my constructor.
 * <p>
 * Extends AbandonedTrace to implement Connection tracking and logging of code
 * which created the Connection. Tracking the Connection ensures that the
 * AbandonedObjectPool can close this connection and recycle it if its pool of
...
public class DelegatingConnection<C extends Connection> extends AbandonedTrace implements Connection {
	...
	/**
	 * Create a wrapper for the Connection which traces this Connection in the
	 * AbandonedObjectPool.
	 *
	 * @param c
	 ...
	 * {@code DelegatingConnection}, returns it, otherwise recursively invokes
	 * this method on my delegate.
	 * <p>
	 * Hence this method will return the first delegate that is not a
	 * {@code DelegatingConnection}, or {@code null} when no
	 * non-{@code DelegatingConnection} delegate can be found by traversing this
	 * chain.
	 * <p>
	 * This method is useful when you may have nested
	 * {@code DelegatingConnection}s, and you want to make sure to obtain a
	 * "genuine" {@link Connection}.
	 * 
			...
			throws SQLException
	{
		...
		try {
			final DelegatingPreparedStatement dps = new DelegatingPreparedStatement(this,
					_conn.prepareStatement(sql, resultSetType, resultSetConcurrency));
			initializeStatement(dps);
			return dps;
		} catch (final SQLException e) {
			handleException(e);
			...
			throws SQLException
	{
		...
		try {
			final DelegatingCallableStatement dcs = new DelegatingCallableStatement(this,
					_conn.prepareCall(sql, resultSetType, resultSetConcurrency));
			initializeStatement(dcs);
			return dcs;
		} catch (final SQLException e) {
			handleException(e);
			...
			final int resultSetHoldability) throws SQLException
	{
		...
		try {
			final DelegatingStatement ds = new DelegatingStatement(this,
					_conn.createStatement(resultSetType, resultSetConcurrency, resultSetHoldability));
			initializeStatement(ds);
			return ds;
		} catch (final SQLException e) {
			handleException(e);
			...
			final int resultSetHoldability) throws SQLException
	{
		...
		try {
			final DelegatingPreparedStatement dps = new DelegatingPreparedStatement(this,
					_conn.prepareStatement(sql, resultSetType, resultSetConcurrency, resultSetHoldability));
			initializeStatement(dps);
			return dps;
		} catch (final SQLException e) {
			handleException(e);
			...
			final int resultSetHoldability) throws SQLException
	{
		...
		try {
			final DelegatingCallableStatement dcs = new DelegatingCallableStatement(this,
					_conn.prepareCall(sql, resultSetType, resultSetConcurrency, resultSetHoldability));
			initializeStatement(dcs);
			return dcs;
		} catch (final SQLException e) {
			handleException(e);
	...
	public PreparedStatement prepareStatement(final String sql, final int autoGeneratedKeys) throws SQLException
	{
		...
		try {
			final DelegatingPreparedStatement dps = new DelegatingPreparedStatement(this,
					_conn.prepareStatement(sql, autoGeneratedKeys));
			initializeStatement(dps);
			return dps;
		} catch (final SQLException e) {
			handleException(e);
	...
	public PreparedStatement prepareStatement(final String sql, final int columnIndexes[]) throws SQLException
	{
		...
		try {
			final DelegatingPreparedStatement dps = new DelegatingPreparedStatement(this,
					_conn.prepareStatement(sql, columnIndexes));
			initializeStatement(dps);
			return dps;
		} catch (final SQLException e) {
			handleException(e);
