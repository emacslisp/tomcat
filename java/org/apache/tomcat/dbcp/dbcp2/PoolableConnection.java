...
public class PoolableConnection extends DelegatingConnection<Connection> implements PoolableConnectionMXBean {
	 ...
	 * connection. Such a connection should be considered broken and not pass
	...
	public PoolableConnection(final Connection conn, final ObjectPool<PoolableConnection> pool,
			final ObjectName jmxName, final Collection<String> disconnectSqlCodes, final boolean fastFailValidation) {
		super(conn);
		_pool = pool;
		_jmxName = jmxName;
	 ...
	 * and this connection has previously thrown a fatal disconnection
	 * exception, a {@code SQLException} is thrown.</li>
	 * <li>If {@code sql} is null, the driver's #{@link Connection#isValid(int)
	 * isValid(timeout)} is called. If it returns {@code false},
	 * {@code SQLException} is thrown; otherwise, this method returns
	 * successfully.</li>
	 * <li>If {@code sql} is not null, it is executed as a query and if the
	 * resulting {@code ResultSet} contains at least one row, this method
	 * returns successfully. If not, {@code SQLException} is thrown.</li>
	 * </ol>
	 * 
	 * @param sql
	 *            validation query
	 ...
	 * states are compared to those in the configured list of fatal exception
	 * codes. If this property is not set, codes are compared against the
	 * default codes in #{@link Utils.DISCONNECTION_SQL_CODES} and in this case
	 * anything starting with #{link Utils.DISCONNECTION_SQL_CODE_PREFIX} is
	 * considered a disconnection.
	 * </p>
	 *
	 * @param e
