...
class PooledConnectionImpl
	 ...
	 * <code>PooledConnection</code> so that it may not be used to generate any
	 * more logical <code>Connection</code>s.
	 *
	 * @throws SQLException
	...
	/**
	 * Closes the physical connection and checks that the logical connection was
	 * closed as well.
	 */
	@Override
	...
	/**
	 * Normalize the given SQL statement, producing a canonical form that is
		...
		if (null == key.getResultSetType() && null == key.getResultSetConcurrency()) {
			if (null == key.getAutoGeneratedKeys()) {
				return new DefaultPooledObject<>(new PoolablePreparedStatement<>(
						connection.prepareStatement(key.getSql()), key, pstmtPool, delegatingConnection));
			}
			return new DefaultPooledObject<>(new PoolablePreparedStatement<>(
					connection.prepareStatement(key.getSql(), key.getAutoGeneratedKeys().intValue()), key, pstmtPool,
					delegatingConnection));
		}
		return new DefaultPooledObject<>(new PoolablePreparedStatement<>(connection.prepareStatement(key.getSql(),
				key.getResultSetType().intValue(), key.getResultSetConcurrency().intValue()), key, pstmtPool,
				delegatingConnection));
	}
	 ...
	 */
	@Override
	public void destroyObject(final PStmtKeyCPDS key, final PooledObject<PoolablePreparedStatement<PStmtKeyCPDS>> p)
			throws Exception
	{
	 ...
	 */
	@Override
	public void activateObject(final PStmtKeyCPDS key, final PooledObject<PoolablePreparedStatement<PStmtKeyCPDS>> p)
			throws Exception
	{
	 ...
	 */
	@Override
	public void passivateObject(final PStmtKeyCPDS key, final PooledObject<PoolablePreparedStatement<PStmtKeyCPDS>> p)
			throws Exception
	{
