 ...
 * {@link javax.sql.ConnectionPoolDataSource}, but still include a
 * {@link java.sql.DriverManager} implementation.
 * <code>ConnectionPoolDataSource</code>s are not used within general
 * applications. They are used by <code>DataSource</code> implementations that
 * pool <code>Connection</code>s, such as
 ...
 * <code>ConnectionPoolDataSource</code> whose attributes are presented as bean
 * getters/setters and then deploying it via JNDI. It is then available as a
 * source of physical connections to the database, when the pooling
 ...
 * be instantiated and initialized as any bean and then attached directly to a
 * pooling <code>DataSource</code>. <code>Jdbc2PoolDataSource</code> can use the
 ...
 * <code>ConnectionPoolDataSource</code> implementation, but is addressed within
 * the jdbc3 specification. The <code>PreparedStatement</code> pool in
 * DriverAdapterCPDS has been in the dbcp package for some time, but it has not
 * undergone extensive testing in the configuration used here. It should be
 * considered experimental and can be toggled with the poolPreparedStatements
 * attribute.
 * </p>
 *
 * <p>
 ...
 * documentation</a> shows how to use <code>DriverAdapterCPDS</code> as a source
 * for <code>Jdbc2PoolDataSource</code> without the use of JNDI.
 * </p>
 *
...
public class DriverAdapterCPDS implements ConnectionPoolDataSource, Referenceable, Serializable, ObjectFactory {
	...
	public PooledConnection getPooledConnection(final String username, final String pass) throws SQLException
	{
		...
		if (isPoolPreparedStatements()) {
			...
			if (getMaxPreparedStatements() <= 0) {
				...
				// pool has LRU functionality so when the limit is reached, 15%
				// of the pool is cleared.
				// see
				// org.apache.tomcat.dbcp.pool2.impl.GenericKeyedObjectPool.clearOldest
				// method
	...
	/**
	 * Gets the value of description. This property is here for use by the code
	...
	/**
	 * Sets the value of description. This property is here for use by the code
	 * which will deploy this datasource. It is not used internally.
	 *
	 * @param v
	...
	/**
	 * Sets the driver classname. Setting the driver classname cause the driver
	 * to be registered with the DriverManager.
	 * 
	 * @param v
	...
	/**
	 * Gets the number of milliseconds to sleep between runs of the idle object
