...
public class Http2AsyncUpgradeHandler extends Http2UpgradeHandler {
	...
	protected void writeSettings()
	{
		...
		socketWrapper.write(BlockingMode.SEMI_BLOCK, getWriteTimeout(), TimeUnit.MILLISECONDS, null,
	...
	void writeBody(Stream stream, ByteBuffer data, int len, boolean finished) throws IOException
	{
		...
		if (writeable) {
			...
			socketWrapper.write(BlockingMode.BLOCK, getWriteTimeout(), TimeUnit.MILLISECONDS, null,
	...
	void writeWindowUpdate(Stream stream, int increment, boolean applicationInitiated) throws IOException
	{
		...
		socketWrapper.write(BlockingMode.SEMI_BLOCK, getWriteTimeout(), TimeUnit.MILLISECONDS, null,
				SocketWrapperBase.COMPLETE_WRITE, errorCompletion, ByteBuffer.wrap(frame), ByteBuffer.wrap(frame2));
		handleAsyncException();
	}
	...
	private class AsyncHeaderFrameBuffers implements HeaderFrameBuffers {
		...
		{
			socketWrapper.write(BlockingMode.SEMI_BLOCK, getWriteTimeout(), TimeUnit.MILLISECONDS, null,
					SocketWrapperBase.COMPLETE_WRITE, applicationErrorCompletion, bufs.toArray(BYTEBUFFER_ARRAY));
			handleAsyncException();
		}
