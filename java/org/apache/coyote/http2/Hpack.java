...
final class Hpack {
	...
	private static final int MAX_INTEGER_OCTETS = 8; // not sure what a good
														// value for this is,
														// but the spec says we
														// need to provide an
														// upper bound

	/**
	 * table that contains powers of two, used as both bitmask and to quickly
	 * calculate 2^n
	 */
	 ...
	 * it means that there was not enough data in the buffer to complete the
	 * decoding sequence.
	 * <p/>
	 * If this method returns -1 then the source buffer will not have been
	 * modified.
	 *
	 * @param source
	 *            The buffer that contains the integer
	 * @param n
	 *            The encoding prefix length
	 ...
	 * <p/>
	 * This method assumes that the buffer has already had the first 8-n bits
	 * filled. As such it will modify the last byte that is already present in
	 * the buffer, and potentially add more if required
	 *
	 * @param source
	 *            The buffer that contains the integer
	 * @param value
