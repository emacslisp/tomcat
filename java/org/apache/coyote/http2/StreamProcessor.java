...
class StreamProcessor extends AbstractProcessor {
						...
						if (!getErrorState().isConnectionIoAllowed()) {
							...
							StreamException se = new StreamException(
									sm.getString("streamProcessor.error.stream", stream.getConnectionId(),
											stream.getIdentifier()),
									Http2Error.INTERNAL_ERROR, stream.getIdentifier().intValue());
							stream.close(se);
						}
					}
				} catch (Exception e) {
					ConnectionException ce = new ConnectionException(sm.getString("streamProcessor.error.connection",
							stream.getConnectionId(), stream.getIdentifier()), Http2Error.INTERNAL_ERROR);
					ce.initCause(e);
					stream.close(ce);
				} finally {
	...
	protected final void executeDispatches()
	{
		...
		synchronized (this) {
			/*
			 * TODO Check if this sync is necessary. Compare with superclass
			 * that uses SocketWrapper
			 */
