...
class Stream extends AbstractStream implements HeaderEmitter {
	...
	public final void emitHeader(String name, String value) throws HpackException
	{
		...
		if (pseudoHeader && headerState != HEADER_STATE_PSEUDO) {
			headerStateErrorMsg = sm.getString("stream.header.unexpectedPseudoHeader", getConnectionId(),
					getIdentifier(), name);
		...
		default: {
			...
			if (pseudoHeader) {
				headerStateErrorMsg = sm.getString("stream.header.unknownPseudoHeader", getConnectionId(),
						getIdentifier(), name);
			}
	...
	public void validateHeaders() throws StreamException
	{
		...
		throw new StreamException(headerStateErrorMsg, Http2Error.PROTOCOL_ERROR, getIdentifier().intValue());
	}
	...
	final void writeHeaders() throws IOException
	{
		...
		boolean endOfStream = getOutputBuffer().hasNoBody() && coyoteResponse.getTrailerFields() == null;
	...
	class StreamInputBuffer implements InputBuffer {
		 ...
		 * that a request body frame could be received before the application is
		 * ready to read it. If it isn't buffered, processing of the connection
		 * (and hence all streams) would block until the application read the
		 * data. Hence the incoming data has to be buffered. If only one buffer
		 * was used then it could become corrupted if the connection thread is
		 * trying to add to it at the same time as the application is read it.
		 * While it should be possible to avoid this corruption by careful use
		 * of the buffer it would still require the same copies as using two
		 * buffers and the behaviour would be less clear.
		 *
