...
public class HpackDecoder {
	 ...
	 * construct as it would be silly to actually shuffle the items around in
	 * the array.
	 */
	private int firstSlotPosition = 0;

	/**
	 * The current table size by index (aka the number of index positions that
	 * are filled up)
	 */
	 ...
	 * there is no more data in which case this should be considered a protocol
	 * error.
	 *
	 * @param buffer
	 *            The buffer
	 ...
	 *             If the packed data is not valid
	 */
	...
	private void resizeIfRequired()
	{
		if (filledTableSlots == headerTable.length) {
			Hpack.HeaderField[] newArray = new Hpack.HeaderField[headerTable.length + 10]; // we
																							// only
																							// grow
																							// slowly
	...
	interface HeaderEmitter {
		 ...
		 *            Header value
		 * @throws HpackException
		 *             If a header is received that is not compliant with the
		 ...
		 *             If the headers received to date are not valid
		 */
