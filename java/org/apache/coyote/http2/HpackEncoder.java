...
class HpackEncoder {
	...
	private static final HpackHeaderFunction DEFAULT_HEADER_FUNCTION = new HpackHeaderFunction() {
		...
		{
			return header.length() > 5; // TODO: figure out a good value for
										// this
		}
	...
	private int minNewMaxHeaderSize = -1; // records the smallest value of
											// newMaxHeaderSize, as per section
											// 4.1
	...
	private final Map<String, List<TableEntry>> dynamicTable = new HashMap<>(); // TODO:
																				// use
																				// a
																				// custom
																				// data
	 ...
	 * @param target
	 *            The buffer to which to write the encoded headers
	 *
	...
	private void writeHuffmanEncodableName(ByteBuffer target, String headerName)
	{
		...
		target.put((byte) 0); // to use encodeInteger we need to place the first
								// byte in the buffer.
	...
	private TableEntry findInTable(String headerName, String value)
	{
		...
		if (staticTable != null) {
			for (TableEntry st : staticTable) {
				if (st.value != null && st.value.equals(value)) { // todo: some
																	// form of
																	// lookup?
					return st;
				}
