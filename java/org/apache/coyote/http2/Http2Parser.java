...
class Http2Parser {
	 ...
	 *
	 * @param block
	 *            Should this method block until a frame is available if no
	 ...
	 *
	 * @throws IOException
	 *             If an IO error occurs while trying to read a frame
	 */
		...
		if (dest == null) {
			...
			synchronized (dest) {
				if (dest.remaining() < dataLength) {
					...
					throw new StreamException("Client sent more data than stream window allowed",
	...
	private void readHeaderPayload(int streamId, int payloadSize) throws Http2Exception, IOException
	{
		...
		while (read < len) {
			...
			if (mustBeZero) {
				...
				for (int i = 0; i < thisTime; i++) {
					if (buffer[i] != 0) {
						throw new ConnectionException(
								sm.getString("http2Parser.nonZeroPadding", connectionId, Integer.toString(streamId)),
								Http2Error.PROTOCOL_ERROR);
					}
				}
			}
	...
	/*
	 * Implementation note: Validation applicable to all incoming frames should
	 * be implemented here. Frame type specific validation should be performed
	 * in the appropriate readXxxFrame() method. For validation applicable to
	 * some but not all frame types, use your judgement.
	 */
	...
	static interface Input {
		 ...
		 *            Buffer to fill
		 * @param offset
		 *            Position in buffer to start writing
		 * @param length
		 ...
		 *
		 * @throws IOException
		 *             If an I/O occurred while obtaining data with which to
		 *             fill the buffer
		 */
