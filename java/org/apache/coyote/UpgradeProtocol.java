...
public interface UpgradeProtocol {
	...
	/**
	 * @param isSSLEnabled
	 *            Is this for a connector that is configured to support TLS.
	 *            Some protocols (e.g. HTTP/2) only support HTTP upgrade over
	 ...
	 * the identifier is not the UTF-8 encoding of the name then some
	 * refactoring is going to be required.
	 *
	 * Implementation note: Tomcat assumes that the UTF-8 encoding of this name
	 * will not exceed 255 bytes. Tomcat's behaviour if longer names are used is
	 * undefined.
	 */
	...
	/**
	 * @param socketWrapper
	 *            The socketWrapper for the connection that requires a processor
	 * @param adapter
	 ...
	 *            The socket
	 * @param adapter
	 *            The Adapter to use to configure the new upgrade handler
	 * @param request
	 *            A copy (may be incomplete) of the request that triggered the
	 *            upgrade
	 *
	 ...
	 *
	 * @param request
	 *            The request that included an upgrade header for this protocol
	 *
