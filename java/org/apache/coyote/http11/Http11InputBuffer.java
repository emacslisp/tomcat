...
public class Http11InputBuffer implements InputBuffer, ApplicationBufferHandler {
	 ...
	 *             if the supplied filter is null
	 */
	...
	boolean isFinished()
	{
		...
		/*
		 * Don't use fill(false) here because in the following circumstances BIO
		 * will block - possibly indefinitely - client is using keep-alive and
		 * connection is still open - client has sent the complete request -
		...
		if (lastActiveFilter >= 0) {
			...
			// No filters. Assume request is not finished. EOF will signal end
			// of
			// request.
			return false;
		}
	...
	private HeaderParseStatus skipLine() throws IOException
	{
		...
		if (log.isDebugEnabled()) {
			log.debug(sm.getString("iib.invalidheader", new String(byteBuffer.array(), headerData.start,
					headerData.lastSignificantChar - headerData.start + 1, StandardCharsets.ISO_8859_1)));
		}
