...
public class RestCsrfPreventionFilter extends CsrfPreventionFilterBase {
	...
	private class StateChangingRequest implements RestCsrfPreventionStrategy {
		...
		{
			if (isValidStateChangingRequest(extractNonceFromRequest(request), nonceFromSession
					.getNonce(request.getSession(false), Constants.CSRF_REST_NONCE_SESSION_ATTR_NAME))) {
				return true;
			}
