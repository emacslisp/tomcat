...
public class Connector extends LifecycleMBeanBase {
	 ...
	 *         automatically parsed by the container. A value of less than 0
	 *         means no limit.
	 */
	 ...
	 *         If the special value for {@link #getPort} of zero is used then
	 *         this method will report the actual port bound.
	 */
	 ...
	 *         comes in on a non-SSL port and is subject to a security
	 *         constraint with a transport guarantee that requires SSL.
	 */
	...
	public void setURIEncoding(String URIEncoding)
	{
		try {
			...
			log.warn(sm.getString("coyoteConnector.invalidEncoding", URIEncoding, uriCharset.name()), e);
		}
	}
	...
	protected void initInternal() throws LifecycleException
	{
		...
		if (AprLifecycleListener.isAprAvailable() && AprLifecycleListener.getUseOpenSSL()
			...
			if (jsseProtocolHandler.isSSLEnabled() && jsseProtocolHandler.getSslImplementationName() == null) {
				// OpenSSL is compatible with the JSSE configuration, so use it
				// if APR is available
