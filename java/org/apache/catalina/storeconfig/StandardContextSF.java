...
public class StandardContextSF extends StoreFactoryBase {
	...
	protected void storeContextSeparate(PrintWriter aWriter, int indent, StandardContext aContext) throws Exception
	{
		...
		if (configFile != null) {
			...
			if ((!config.isFile()) || (!config.canWrite())) {
				log.error("Cannot write context output file at " + configFile + ", not saving.");
				throw new IOException("Context save file at " + configFile + " not a file, or not writable.");
			}
			if (log.isInfoEnabled())
				log.info("Store Context " + aContext.getPath() + " separate at file " + config);
	...
	protected void storeWithBackup(StandardContext aContext) throws Exception
	{
		...
		if (mover != null) {
			...
			if ((mover.getConfigOld() == null) || (mover.getConfigOld().isDirectory())
				...
				log.error("Cannot move orignal context output file at " + mover.getConfigOld());
				throw new IOException(
						"Context orginal file at " + mover.getConfigOld() + " is null, not a file or not writable.");
			}
			File dir = mover.getConfigSave().getParentFile();
			if (dir != null && dir.isDirectory() && (!dir.canWrite())) {
				log.error("Cannot save context output file at " + mover.getConfigSave());
				throw new IOException("Context save file at " + mover.getConfigSave() + " is not writable.");
			}
			if (log.isInfoEnabled())
				log.info("Store Context " + aContext.getPath() + " separate with backup (at file "
						+ mover.getConfigSave() + " )");
	 ...
	 *            The context instance
	 * @param wresources
	 *            The raw watched resources list
	 * @return The filtered watched resources
	 * @throws Exception
	 *             Configuration storing error TODO relative watched resources
	 *             TODO absolute handling configFile TODO Filename case handling
	 *             for Windows? TODO digester variable substitution
	...
	protected String[] filterWatchedResources(StandardContext context, String[] wresources) throws Exception
	{
		...
		String configFile = (context.getConfigFile() != null
				? new File(context.getConfigFile().toURI()).getCanonicalPath() : null);
