...
public class StoreLoader {
	 ...
	 * 
	 * @param aFile
	 *            File name, absolute or relative to
	 *            <code>${catalina.base}/conf</code>, if not specified
	...
	public void load()
	{
		...
		try {
			...
			if (configUrl != null) {
				...
				if (log.isInfoEnabled())
					log.info("Find registry server-registry.xml from system property at url " + configUrl);
		...
		if (is == null) {
			try {
				...
				if (log.isInfoEnabled())
					log.info("Find registry server-registry.xml at file " + reg.getCanonicalPath());
