...
public class StoreConfig implements IStoreConfig {
	 ...
	 *            Server ObjectName
	 * @param backup
	 *            <code>true</code> to backup existing configuration files
	 *            before rewriting them
	 * @param externalAllowed
	 *            <code>true</code> to allow saving webapp configuration for
	 *            webapps that are not inside the host's app directory
		...
		if (mserver.isRegistered(objectName)) {
			try {
				...
				if (configFile != null) {
					...
					log.error("Missing configFile at Context " + aContext.getPath() + " to store!");
			} catch (Exception e) {
				if (log.isInfoEnabled())
					log.info("Object " + aContextName + " is no a context instance or store exception", e);
			}
		} else if (log.isInfoEnabled())
	...
	{
		StoreFileMover mover = new StoreFileMover(System.getProperty("catalina.base"), getServerFilename(),
