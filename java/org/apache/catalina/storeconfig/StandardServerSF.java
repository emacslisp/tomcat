...
public class StandardServerSF extends StoreFactoryBase {
			...
			throws Exception
	{
		if (aObject instanceof StandardServer) {
			...
			/*
			 * LifecycleListener listener = null; for (int i = 0; listener ==
			 * null && i < listeners.length; i++) if (listeners[i] instanceof
			 * ServerLifecycleListener) listener = listeners[i]; if (listener !=
			 * null) { StoreDescription elementDesc = getRegistry()
			 * .findDescription( StandardServer.class.getName() +
			 * ".[ServerLifecycleListener]"); if (elementDesc != null) {
			 * elementDesc.getStoreFactory().store(aWriter, indent, listener); }
			 * }
			 */
			...
			StoreDescription elementDesc = getRegistry()
					.findDescription(NamingResourcesImpl.class.getName() + ".[GlobalNamingResources]");
			if (elementDesc != null) {
