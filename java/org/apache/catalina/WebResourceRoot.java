 ...
 * application's context.xml. Resources will be searched in the order they were
 * specified.</li>
 ...
 * ResourceSet. The write operation will fail if the presence of a Resource in
 * one of the other ResourceSets effectively makes the operation on the main
 * ResourceSet a NO-OP.</li>
 * <li>A file in a ResourceSet will hide a directory of the same name (and all
 * the contents of that directory) in a ResourceSet that is later in the search
 * order.</li>
 ...
 * in earlier versions:
 * <ul>
 * <li>Aliases - Replaced by Post-Resources with the addition of support for
 * single files as well as directories and JARs.</li>
 ...
 * WEB-INF/lib and WEB-INF/classes</li>
 * <li>Resource JARs - Same feature but implemented using the same mechanism as
 * all the other additional resources.</li>
 * </ul>
 */
/*
 * A potential future enhancement is to allow writing to any ResourceSet, not
 * just the main ResourceSet although that adds all sorts complications
 * including: - which ResourceSet to write to - unexpected behaviour when
 * deleting a resource from one ResourceSet since that may unmask a resource in
 * a lower priority ResourceSet so what was a delete looks like a replace with
 * the user having no idea where the 'new' resource came from - how to handle
 * PUT when the target is read-only but it could be written to a higher priority
 * ResourceSet that is read-write
 */
public interface WebResourceRoot extends Lifecycle {
	/**
	 * Obtain the object that represents the resource at the given path. Note
	 * that the resource at that path may not exist. If the path does not exist,
	 * the WebResource returned will be associated with the main WebResourceSet.
	 *
	 * @param path
	 *            The path for the resource of interest relative to the root of
	 *            the web application. It must start with '/'.
	 *
	 ...
	 * that the resource at that path may not exist. If the path does not exist,
	 * the WebResource returned will be associated with the main WebResourceSet.
	 * This will include all matches even if the resource would not normally be
	 * accessible (e.g. because it was overridden by another resource)
	 *
	 * @param path
	 *            The path for the resource of interest relative to the root of
	 *            the web application. It must start with '/'.
	 ...
	 *
	 * @param path
	 *            The path of the class loader resource of interest relative to
	 *            the the root of class loader resources for this web
	 *            application.
	 *
	 * @return The object that represents the class loader resource at the given
	 *         path
	 */
	 ...
	 *
	 * @param path
	 *            The path for the class loader resource of interest relative to
	 *            the root of the class loader resources for the web
	 ...
	 *
	 * @param path
	 *            The path to be used for the new Resource. It is relative to
	 *            the root of the web application and must start with '/'.
	 * @param is
	 *            The InputStream that will provide the content for the new
	 *            Resource.
	 * @param overwrite
	 *            If <code>true</code> and the resource already exists it will
	 *            be overwritten. If <code>false</code> and the resource already
	 *            exists the write will fail.
	 *
	 ...
	 *
	 * @param type
	 *            The type of {@link WebResourceSet} to create
	 * @param webAppMount
	 *            The path within the web application that the resources should
	 *            be published at. It must start with '/'.
	 * @param url
	 *            The URL of the resource (must locate a JAR, file or directory)
	 * @param internalPath
	 *            The path within the resource where the content is to be found.
	 *            It must start with '/'.
	 */
	 ...
	 * @param archivePath
	 *            The path within the resource to the archive where the content
	 *            is to be found. If there is no archive then this should be
	 *            <code>null</code>.
	 * @param internalPath
	 *            The path within the archive (or the resource if the
	 *            archivePath is <code>null</code> where the content is to be
	 *            found. It must start with '/'.
	 */
	...
	/**
	 * @return the list of WebResourceSet configured to this web application as
	 *         a 'Pre' resource.
	 */
	 ...
	 *            <code>true</code> to enable caching, else <code>false</code>
	 */
	 ...
	 *            {@code true} to enable it, {@code false} to disable it
	 */
	...
	/**
	 * Add a specified resource to track to be able to later release resources
	 * on stop.
	 * 
