...
public class RewriteRule {
	...
	/**
	 * This flag chains the current rule with the next rule (which itself can be
	 * chained with the following rule, etc.). This has the following effect: if
	 * a rule matches, then processing continues as usual, i.e., the flag has no
	 * effect. If the rule does not match, then all following chained rules are
	 * skipped. For instance, use it to remove the ``.www'' part inside a
	 * per-directory rule set when you let an external redirect happen (where
	 * the ``.www'' part should not to occur!).
	 */
	protected boolean chain = false;

	/**
	 * This sets a cookie on the client's browser. The cookie's name is
	 * specified by NAME and the value is VAL. The domain field is the domain of
	 * the cookie, such as '.apache.org',the optional lifetime is the lifetime
	 * of the cookie in minutes, and the optional path is the path of the cookie
	 */
	 ...
	 * from the last rewriting rule. This corresponds to the Perl next command
	 * or the continue command from the C language. Use this flag to restart the
	 * rewriting process, i.e., to immediately go to the top of the loop. But be
	 ...
	 * RewriteRule /foo/(.*) /bar?arg=P1\%3d$1 [R,NE] which would turn
	 * '/foo/zed' into a safe request for '/bar?arg=P1=zed'.
	 */
	 ...
	 * occur internally in Apache when mod_include tries to find out information
	 * about possible directory default files (index.xxx). On sub-requests it is
	 * not always useful and even sometimes causes a failure to if the complete
	 * set of rules are applied. Use this flag to exclude some rules. Use the
	 * following rule for your decision: whenever you prefix some URLs with
	 * CGI-scripts to force them to be processed by the CGI-script, the chance
	 * is high that you will run into problems (or even overhead) on
	 ...
	 * URL a URI) to force a external redirection. If no code is given an HTTP
	 * response of 302 (FOUND, previously MOVED TEMPORARILY) is used. If you
	 * want to use other response codes in the range 300-399 just specify them
	 * as a number or use one of the following symbolic names: temp (default),
	 * permanent, seeother. Use it for rules which should canonicalize the URL
	 * and give it back to the client, e.g., translate ``/~'' into ``/u/'' or
	 * always append a slash to /u/user, etc. Note: When you use this flag, make
	 * sure that the substitution field is a valid URL! If not, you are
	 ...
	 * if-then-else constructs: The last rule of the then-clause becomes skip=N
	 * where N is the number of rules in the else-clause. (This is not the same
	 * as the 'chain|C' flag!)
	 */
	 ...
	 * this can be used to setup the content-type based on some conditions. For
	 * example, the following snippet allows .php files to be displayed by
	 * mod_php if they are called with the .phps extension: RewriteRule
	 * ^(.+\.php)s$ $1 [T=application/x-httpd-php-source]
	 */
