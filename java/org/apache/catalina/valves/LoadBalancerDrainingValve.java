...
/**
 * <p>
 * A Valve to detect situations where a load-balanced node receiving a request
 * has been deactivated by the load balancer (JK_LB_ACTIVATION=DIS) and the
 * incoming request has no valid session.
 * </p>
 *
 * <p>
 * In these cases, the user's session cookie should be removed if it exists, any
 * ";jsessionid" parameter should be removed from the request URI, and the
 * client should be redirected to the same URI. This will cause the
 * load-balanced to re-balance the client to another server.
 * </p>
 *
 * <p>
 * A request parameter is added to the redirect URI in order to avoid repeated
 * redirects in the event of an error or misconfiguration.
 * </p>
 *
 * <p>
 * All this work is required because when the activation state of a node is
 ...
 * whether the session id is valid, it will send the request blindly to the
 * disabled node, which makes it take much longer to drain the node than
 * strictly necessary.
 * </p>
 *
 * <p>
 * For testing purposes, a special cookie can be configured and used by a client
 * to ignore the normal behavior of this Valve and allow a client to get a new
 * session on a DISABLED node. See {@link #setIgnoreCookieName} and
 * {@link #setIgnoreCookieValue} to configure those values.
 * </p>
 *
 * <p>
 * This Valve should be installed earlier in the Valve pipeline than any
 ...
 *
 * @see <a href=
 *      "http://tomcat.apache.org/connectors-doc/generic_howto/loadbalancers.html">Load
 *      balancer documentation</a>
 */
public class LoadBalancerDrainingValve extends ValveBase {
	/**
	 * The request attribute key where the load-balancer's activation state can
	 * be found.
	 */
	public static final String ATTRIBUTE_KEY_JK_LB_ACTIVATION = "JK_LB_ACTIVATION";

	/**
	 * The HTTP response code that will be used to redirect the request back to
	 * the load-balancer for re-balancing. Defaults to 307 (TEMPORARY_REDIRECT).
	 *
	 ...
	 *
	 * @param code
	 *            The code to use for the redirect
	 */
	...
	/**
	 * Gets the name of the cookie that can be used to override the re-balancing
	 * behavior of this Valve when the current node is in the DISABLED
	 * activation state.
	 *
	...
	/**
	 * Sets the name of the cookie that can be used to override the re-balancing
	 * behavior of this Valve when the current node is in the DISABLED
	 * activation state.
	 *
	 * There is no default value for this setting: the ability to override the
	 * re-balancing behavior of this Valve is <i>disabled</i> by default.
	 *
	 * @param cookieName
	 *            The cookie name to use to ignore normal processing rules.
	 *
	 ...
	 * re-balancing behavior of this Valve when the current node is in the
	 ...
	 * re-balancing behavior of this Valve when the current node is in the
	 * DISABLED activation state. The "ignore" cookie's value <b>must</b> be
	 * exactly equal to this value in order to allow the client to override the
	 * re-balancing behavior.
	 *
	 * @param cookieValue
	 *            The cookie value to use to ignore normal processing rules.
	 *
	 * @see #getIgnoreCookieValue
	 */
		...
		if ("DIS".equals(request.getAttribute(ATTRIBUTE_KEY_JK_LB_ACTIVATION))
			...
			if (null != cookies) {
				for (Cookie cookie : cookies) {
					...
					if (null != _ignoreCookieName && _ignoreCookieName.equals(cookieName) && null != _ignoreCookieValue
			...
			if (null != sessionCookie) {
				...
				if (request.getContext().getSessionCookiePathUsesTrailingSlash()) {
					// Handle special case of ROOT context where cookies require
					// a path of
					// '/' but the servlet spec uses an empty string
					// Also ensure the cookies for a context with a path of /foo
					// don't get
